<nav class="app-navbar" x-data="{ open: false }" :class="{ 'menu-open': open }"
    ...
    <div class="container">
        ...
        <div class="app-mobile-menu" :class="{ 'is-open': open }">
            ...
            <div class="app-mobile-links">
                ...
                <a href="{{ route('dashboard') }}"  @click.prevent="window.location.href = $el.href"
                    class="app-nav-link {{ request()->routeIs('dashboard') ? 'active' : '' }}">
                    Dashboard
                </a>

                <a href="{{ route('clientes.index') }}"  @click.prevent="window.location.href = $el.href"
                    class="app-nav-link {{ request()->routeIs('clientes.*') ? 'active' : '' }}">
                    Clientes
                </a>

                <a href="{{ route('viaturas.index') }}"  @click.prevent="window.location.href = $el.href"
                    class="app-nav-link {{ request()->routeIs('viaturas.*') ? 'active' : '' }}">
                    Viaturas
                </a>

                <a href="{{ route('vendas.index') }}"  @click.prevent="window.location.href = $el.href"
                    class="app-nav-link {{ request()->routeIs('vendas.*') ? 'active' : '' }}">
                    Vendas
                </a>

                <a href="{{ route('auditoria.index') }}"  @click.prevent="window.location.href = $el.href"
                    class="app-nav-link {{ request()->routeIs('auditoria.*') ? 'active' : '' }}">
                    Auditoria
                </a>

                @can('gerir-utilizadores')
                    <a href="{{ route('utilizadores.index') }}"  @click.prevent="window.location.href = $el.href"
                        class="app-nav-link {{ request()->routeIs('utilizadores.*') ? 'active' : '' }}">
                        Utilizadores
                    </a>
                @endcan
            ...
            <div class="app-mobile-user">
                <a href="{{ route('profile.edit') }}"  @click.prevent="window.location.href = $el.href" class="app-user-link app-user-link-mobile">
                    {{ Auth::user()->name }}
                </a>
